...
class ReplayBuffer(object):
	def __init__(self, size, view_size):
		...
		self.len = 0
		self.top = 0
		self.s = np.zeros((size, 1, view_size.w, view_size.h), dtype=theano.config.floatX)
		self.a = np.zeros((size, 1), dtype=np.int32)
		self.s2 = np.zeros((size, 1, view_size.w, view_size.h), dtype=theano.config.floatX)
	...
	def draw_batch(self, batch_size):
		...
		return self.s[indices], self.a[indices], self.s2[indices], self.r[indices], self.t[indices]
...
class DeepQAgentGpu(RobotAgent):
	...
	def __init__(self, logger, actions, batch_size, buffer_size, start_learn, learn_steps, view_size, model):
		self.logger = logger
		self.actions = actions
		self.batch_size = batch_size
		buffer_size = max(buffer_size, batch_size)  # buffer has to be at least batch_size
		self.replay_buffer = ReplayBuffer(buffer_size, view_size)
	...
	def choose_action(self, curr_state, eps):
		qs = self.model.predict_qs(curr_state.view)
	...
	def incorporate_reward(self, old_state, action, new_state, reward):
		...
		old_state = old_state.view
		is_terminal = new_state is None
		terminal = 1 if is_terminal else 0
		new_state = new_state.view if not is_terminal else np.zeros(old_state.shape)
		exp_new = (old_state, action, new_state, reward, terminal)
